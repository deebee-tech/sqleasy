...
describe("PostgresSqlEasy where", () => {
   ...
   it("where with AND", () => {
      ...
      const sql = builder.parseRaw();
      expect(sql).toEqual('SELECT * FROM "public"."users" AS "u" WHERE "u"."age" > 18 AND "u"."active" = true;');
   });
   ...
   it("where with OR", () => {
      ...
      const sql = builder.parseRaw();
      expect(sql).toEqual('SELECT * FROM "public"."users" AS "u" WHERE "u"."role" = admin OR "u"."role" = superadmin;');
   });
   ...
   it("whereNotInValues", () => {
      const sqlEasy = new PostgresSqlEasy();
      const builder = sqlEasy.newBuilder();
      builder.selectAll().fromTable("users", "u").whereNotInValues("u", "status", ["banned", "suspended"]);

      const sql = builder.parseRaw();
      expect(sql).toEqual('SELECT * FROM "public"."users" AS "u" WHERE "u"."status" NOT IN (banned, suspended);');
   });
   ...
   it("whereRaws multiple with AND", () => {
      const sqlEasy = new PostgresSqlEasy();
      const builder = sqlEasy.newBuilder();
      builder.selectAll().fromTable("users", "u").whereRaw('"u"."age" > 18').and().whereRaw('"u"."active" = true');
   ...
   it("where multiple conditions with parse() - $1, $2 placeholders", () => {
      ...
      const sql = builder.parse();
      expect(sql).toEqual('SELECT * FROM "public"."users" AS "u" WHERE "u"."age" > $1 AND "u"."active" = $2;');
   });
   ...
   it("whereNotInValues with parse() - $1, $2 placeholders", () => {
      ...
      const sql = builder.parse();
      expect(sql).toEqual('SELECT * FROM "public"."users" AS "u" WHERE "u"."status" NOT IN ($1, $2);');
   });
});
