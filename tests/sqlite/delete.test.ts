...
describe("SqliteSqlEasy delete", () => {
   ...
   it("delete without alias", () => {
      ...
      const sql = builder.parseRaw();
      expect(sql).toEqual(
         'DELETE FROM "temp_data" WHERE "temp_data"."created_at" < 2024-01-01;',
      );
   });
   ...
   it("delete with where null", () => {
      ...
      const sql = builder.parseRaw();
      expect(sql).toEqual(
         'DELETE FROM "users" AS "u" WHERE "u"."email" IS NULL;',
      );
   });
   ...
   it("delete with where in values", () => {
      ...
      const sql = builder.parseRaw();
      expect(sql).toEqual(
         'DELETE FROM "users" AS "u" WHERE "u"."id" IN (1, 2, 3);',
      );
   });
   ...
   it("delete with where in values - parse prepared", () => {
      ...
      const sql = builder.parse();
      expect(sql).toEqual(
         'DELETE FROM "users" AS "u" WHERE "u"."id" IN (?, ?, ?);',
      );
   });
   ...
   it("delete with explicit owner", () => {
      ...
      const builder = sqlEasy.newBuilder();
      builder
         .deleteFromWithOwner("main", "users", "u")
         .where("u", "id", WhereOperator.Equals, 1);

      const sql = builder.parseRaw();
      expect(sql).toEqual(
         'DELETE FROM "main"."users" AS "u" WHERE "u"."id" = 1;',
      );
   });
   ...
   it("delete with where between", () => {
      ...
      builder
         .deleteFrom("logs", "l")
         .whereBetween("l", "id", 100, 200);

      const sql = builder.parseRaw();
      expect(sql).toEqual(
         'DELETE FROM "logs" AS "l" WHERE "l"."id" BETWEEN 100 AND 200;',
      );
   });
});
