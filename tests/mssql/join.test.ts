...
import { JoinOperator, JoinType, MssqlQuery, WhereOperator } from '../../src';

describe('MssqlQuery join', () => {
    ...
    builder
      ...
      .joinWithBuilder(
        ...
        (b) => {
          b.selectAll()
            .fromTable('orders', 'o')
            .where('o', 'status', WhereOperator.Equals, 'active');
        },
        (jb) => {
  ...
  it('ON with value (onValue)', () => {
    ...
    builder
      ...
      .joinTable(JoinType.Inner, 'orders', 'o', (jb) => {
        jb.on('u', 'id', JoinOperator.Equals, 'o', 'user_id')
          .and()
          .onValue('o', 'active', JoinOperator.Equals, 1);
      })
      .where('u', 'id', WhereOperator.Equals, 1);
