...
describe('MssqlQuery where', () => {
  ...
  it('where with LessThanOrEquals', () => {
    ...
    builder
      .selectAll()
      .fromTable('users', 'u')
      .where('u', 'score', WhereOperator.LessThanOrEquals, 100);
  ...
  it('whereNotInValues', () => {
    ...
    builder
      .selectAll()
      .fromTable('users', 'u')
      .whereNotInValues('u', 'status', ['deleted', 'banned']);

    const sql = builder.parseRaw();
    expect(sql).toEqual(
      'SELECT * FROM [dbo].[users] AS [u] WHERE [u].[status] NOT IN (deleted, banned);',
    );
  });
  ...
  it('whereInValues with string values', () => {
    ...
    builder
      .selectAll()
      .fromTable('users', 'u')
      .whereInValues('u', 'status', ['active', 'pending', 'review']);

    const sql = builder.parseRaw();
    expect(sql).toEqual(
      'SELECT * FROM [dbo].[users] AS [u] WHERE [u].[status] IN (active, pending, review);',
    );
  });
});
