...
describe('MysqlQuery where', () => {
  ...
  it('whereInValues with strings', () => {
    ...
    builder
      .selectAll()
      .fromTable('users', 'u')
      .whereInValues('u', 'role', ['admin', 'editor', 'viewer']);

    const sql = builder.parseRaw();
    expect(sql).toEqual(
      'SELECT * FROM `users` AS `u` WHERE `u`.`role` IN (admin, editor, viewer);',
    );
  });
  ...
  it('whereGroup produces grouped parentheses', () => {
    ...
    const sql = builder.parseRaw();
    expect(sql).toEqual(
      'SELECT * FROM `users` AS `u` WHERE `u`.`active` = true AND (`u`.`role` = admin OR `u`.`role` = editor);',
    );
  });
